...
const benefits = [
  "Akses ke semua bab materi TKA Matematika",
  "40+ Simulasi visual & interaktif",
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (typeof window !== "undefined") {
        ...
        if (program) {
          redirectUrl = `/checkout?program=${program}`;
        }
      }
    ...
    <div className="min-h-screen bg-[#f6f8fc]">
      <div className="mx-auto min-h-screen max-w-[768px] px-5 py-5">
        ...
        <p className="mb-7 leading-relaxed text-[#666]">
          Buat akun terlebih dahulu untuk mulai mengakses seluruh materi pembelajaran.
        </p>
        ...
        <div className="rounded-[28px] bg-white p-6 shadow-[0_5px_20px_rgba(0,0,0,0.04)]">
          ...
          <div className="my-5 flex items-center gap-3">
            ...
            <span className="text-xs text-stone-400">atau daftar dengan email</span>
            <div className="h-px flex-1 bg-[#edf1f5]" />
          </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-[18px]">
              <label className="mb-2 block text-sm font-semibold text-stone-800">Nama Lengkap</label>
              <input
                type="text"
                required
                value={form.name}
            ...
            {/* Benefit box */}
            <div className="mt-5 rounded-[18px] bg-[#f8fafc] p-[18px]">
              <p className="mb-3 text-sm font-bold text-stone-800">Yang Kamu Dapatkan</p>
              {benefits.map((item) => (
            ...
            {success && <div className="mt-4"><Alert type="success" message="Akun berhasil dibuat! Mengarahkan ke pembayaran..." /></div>}

            <button
              type="submit"
              disabled={loading || success}
              className="mt-5 h-[58px] w-full rounded-[18px] bg-[#2563eb] text-[15px] font-bold text-white transition-opacity disabled:opacity-60"
            >
              {loading
          ...
          <p className="mt-5 text-center text-sm text-[#666]">
            Sudah punya akun?{" "}
            <Link href="/login" className="font-bold text-[#2563eb]">Masuk</Link>
          </p>
        </div>
